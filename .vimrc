...
hi statusline ctermbg=NONE ctermfg=253
hi User1 term=standout cterm=standout ctermfg=blue
...
set statusline+=\ \|\ %1*%t%*
set statusline+=%1*\>%{get(b:,'coc_current_function','')}%*
set statusline+=\ %m
set statusline+=\ %y
...
nmap <silent><buffer> gK :call <SID>show_coc_documentation()<CR>
...
augroup Development
    autocmd!
    au BufNewFile,BufRead *.py,*.sh call SetDevOptions()
augroup END
